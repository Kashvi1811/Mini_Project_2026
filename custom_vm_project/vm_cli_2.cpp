...
namespace {
    ...
    const string MENU_OPT_PROCESS_STATE_VISUALIZER = "6";
    ...
    int handleProcessStateVisualizerCommand(bool open);
    ...
    void printUsage() {
           cout << "Custom VM CLI v" << CLI_VERSION << "\n"
             ...
             << "  vm_cli viewer [--open]\n\n"
             << "  vm_cli process-visualizer [--open]\n\n"
             << "  vm_cli kali-ui [--open]\n\n"
        ...
        while (true) {
            cout << "\n=== VM CLI Menu ===\n"
                 ...
                 << MENU_OPT_VIEWER << ") Viewer path\n"
                  << MENU_OPT_PROCESS_STATE_VISUALIZER << ") Process State Visualizer\n"
                 << MENU_OPT_EXIT << ") Exit\n"
                 << "Select option: ";
            ...
            if (choice == MENU_OPT_PROCESS_STATE_VISUALIZER) {
                handleProcessStateVisualizerCommand(true);
                continue;
            }
    ...
    int runKaliShell() {
        ...
        while (true) {
            ...
            if (cmd == "process-visualizer") {
                bool open = (parts.size() >= 2 && parts[1] == "--open");
                handleProcessStateVisualizerCommand(open);
                continue;
            }
    ...
    int handleProcessStateVisualizerCommand(bool open) {
        const std::string processVisualizerUrl = "https://kashvi1811.github.io/process-viewer-site/";
        std::cout << "Opening Process State Visualizer: " << processVisualizerUrl << std::endl;
        if (open) {
#ifdef _WIN32
            std::string cmd = "start \"\" \"" + processVisualizerUrl + "\"";
            int rc = system(cmd.c_str());
            if (rc != 0) {
                std::cout << "Failed to launch browser. Please open manually: " << processVisualizerUrl << std::endl;
            }
#elif __APPLE__
            std::string cmd = "open '" + processVisualizerUrl + "'";
            int rc = system(cmd.c_str());
            if (rc != 0) {
                std::cout << "Failed to launch browser. Please open manually: " << processVisualizerUrl << std::endl;
            }
#else
            if (commandExists("xdg-open")) {
                std::string cmd = "xdg-open '" + processVisualizerUrl + "'";
                int rc = system(cmd.c_str());
                if (rc != 0) {
                    std::cout << "Failed to launch browser. Please open manually: " << processVisualizerUrl << std::endl;
                }
            } else {
                std::cout << "No browser launcher found. Please open manually: " << processVisualizerUrl << std::endl;
            }
#endif
        }
        return 0;
...
int main(int argc, char** argv) {
    ...
    if (cmd == "process-visualizer") {
        bool open = false;
        if (argc >= 3 && string(argv[2]) == "--open") open = true;
        return handleProcessStateVisualizerCommand(open);
    }
